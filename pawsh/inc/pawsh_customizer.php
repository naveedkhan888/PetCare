...
class pawsh_customizer {
	...
	public function __construct($data) {
		$this->data = $data;
		add_action('customize_register',array($this,'pawsh_customize_register'));
	...
	function custom_admin_style($hook) {
		if ( 'customize.php' == $hook || 'widgets.php' == $hook ) {
			wp_enqueue_style( 'pawsh-customizer-style', get_template_directory_uri() . '/inc/css/pawsh-customizer-style.css',array());
			wp_enqueue_script( 'pawsh-customizer', get_template_directory_uri() . '/inc/js/pawsh-customizer.js', array( 'jquery' ), '20150611', true );
		}
	}	
	public function pawsh_customize_register($customizeObj) {
	...
	public function create_control($value,$key) {
		...
		if ( isset($value['type']) ) {
			...
			switch($value['type']){
				...
				case "switch":
					$args['type'] = $value['type'];
					$inst = new Pawsh_Customize_Switch_Control($this->customizeObj,$id,$args);
					break;
				case "image":
	...
	public static function pawsh_data_load() {

		$data = apply_filters( 'pawsh_customizer_data', array());
...
add_action('init','pawsh_customizer::pawsh_data_load');
...
if(class_exists( 'WP_Customize_control')) {
	...
	class Pawsh_Customize_Switch_Control extends WP_Customize_Control {
		public $type = 'switch';
		public function render_content() {
			...
			<label>
				...
				<div class="switch_options">
					<span class="switch_enable"><?php esc_html_e('Yes','pawsh'); ?></span>
					<span class="switch_disable"><?php esc_html_e('No','pawsh'); ?></span>  
